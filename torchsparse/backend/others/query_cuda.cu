...
__global__ void convert_out_in_map_kernel(const int* out_in_map,
...
__global__ void derive_bit_mask_from_out_in_map_kernel(int* out_in_map,
                                                       int* bitmask,
                                                       int valid_n, int n,
                                                       int kernel_volume,
                                                       int split_mask_num) {
...
at::Tensor hash_query_cuda(const at::Tensor hash_query,
  ...
  in_hash_table.lookup_many(hash_query.data_ptr<int64_t>(), out.data_ptr<int>(),
                            n1);
  return out;
}
...
at::Tensor derive_bitmask_from_out_in_map(const at::Tensor out_in_map,
                                          ...
                                          int valid_n) {
  at::Tensor bitmask =
      torch::full({split_mask_num, out_in_map.size(0)}, -1,
