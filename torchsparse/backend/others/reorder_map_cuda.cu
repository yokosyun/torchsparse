...
__global__ void __launch_bounds__(thd_num)
    reorder_out_in_map_kernel(int* __restrict__ out_in_map,
                              int* __restrict__ reorder_loc,
                              int M,  // node num
                              int kernel_volume, int split_mask_len,
  ...
  reorder_out_in_map[output_row_idx * kernel_volume + output_col_idx] =
      out_in_map[input_row_idx * kernel_volume + output_col_idx];
}

at::Tensor reorder_out_in_map_cuda(torch::Tensor _out_in_map,
  ...
  auto options = torch::TensorOptions()
                     .dtype(_out_in_map.dtype())
                     .device(_out_in_map.device());
  ...
  reorder_out_in_map_kernel<<<(M + cta_M - 1) / cta_M * kernel_volume, cta_M>>>(
      out_in_map, reorder_loc, M, kernel_volume, split_mask_len,
      reorder_out_in_map);
