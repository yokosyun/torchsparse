...
#define output_per_blk \
  32  // thd_per_blk / 4  -> (4 threads for 1 reduced element in int32)

extern "C" __global__ void __launch_bounds__(thd_per_blk)
    reduce_mask_cuda_int32(int* __restrict__ bitmask, int output_node_num,
                           int reduced_row_num, int reduce_tile,
  ...
  int* reduced_bitmask_blk =
      reduced_bitmask + split_mask_iter * reduced_row_num;
...
torch::Tensor reduce_bitmask_cuda(torch::Tensor _bitmask_int, int M_tile) {
  if (M_tile % 4 != 0) {
    throw std::runtime_error(
        "[Bitmask reduce] reduce tile size must be multiple of 4.");
  }
  ...
  auto options =
      torch::TensorOptions().dtype(torch::kInt32).device(_bitmask_int.device());
  torch::Tensor _reduced_bitmask_int =
      torch::zeros({split_mask_num, reduced_row_num}, options);
  ...
  reduce_mask_cuda_int32<<<num_blocks, num_threads>>>(
      bitmask_int, output_node_num, reduced_row_num, M_tile,
      reduced_bitmask_int);
