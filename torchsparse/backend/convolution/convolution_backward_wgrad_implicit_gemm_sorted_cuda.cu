...
__global__ void __launch_bounds__(32)
    conv_backward_cuda_setting1_mode1_f16f16f32(
        ...
        int *__restrict__ reorder_loc, half *__restrict__ C) {
  const int K_tile = 16;
...
#pragma message("FP16 kernels will not be compiled for SM75-.")
  ...
  for (int _i2_0_0 = K_iters - 1; _i2_0_0 < K_iters; ++_i2_0_0) {
    ...
    if (i2_0_0 >= (M_fwd + 63) / 64) continue;
...
template <int K_ld_factor, int N_ld_factor, bool K_ld_check, bool N_ld_check>
__global__ void __launch_bounds__(32)
    conv_backward_cuda_setting1_mode1_tf32tf32f32(
        int M_fwd, int K_original, int N, int kernel_volume, int split_k_iters,
        int split_mask_len, int reduced_mask_len, int reorder_loc_len,
...
template <int K_ld_factor, int N_ld_factor, bool K_ld_check, bool N_ld_check>
__global__ void __launch_bounds__(32)
    conv_backward_cuda_setting1_mode1_f32f32f32(
        int M_fwd, int K_original, int N, int kernel_volume, int split_k_iters,
        int split_mask_len, int reduced_mask_len, int reorder_loc_len,
        float *__restrict__ A, float *__restrict__ B,
        int *__restrict__ reduced_mask, int *__restrict__ out_in_map,
...
at::Tensor conv_backward_wgrad_implicit_gemm_sorted_cuda(
    ...
    torch::Tensor _reduced_mask, torch::Tensor _reorder_loc,
  ...
  if (is_half) {
    // throw std::runtime_error("FP16 kernels have not been updated for split
    // mask implimentation.");
    if (!allow_fp16) {
    ...
    if (num_out_channels % 64 == 0 && num_in_channels % 32 == 0) {
      ...
      dim3 num_blocks(1 * num_in_channels * kernel_volume / 32 * j_factors1 *
                      split_k_iters);
      // threadIdx.x: 32
      // threadIdx.y: i_factors[2] * j_factors[2]
      dim3 threads_per_block(32, 2);
      conv_backward_cuda_setting2_mode1_f16f16f32<<<num_blocks,
                                                    threads_per_block>>>(
      ...
      dim3 num_blocks(1 * (num_in_channels + 15) / 16 * kernel_volume *
                      j_factors1 * split_k_iters);
      // threadIdx.x: 32
      // threadIdx.y: i_factors[2] * j_factors[2]
      dim3 threads_per_block(32, 1);
      // conv_backward_cuda_setting1_mode1_f16f16f32<<<num_blocks,
      // threads_per_block>>>(
      //     _kernel.size(0), num_in_channels, num_out_channels, kernel_volume,
      //     split_k_iters, in_feats, kernel, reduced_mask, out_in_map,
    ...
    // throw std::runtime_error("TF32 kernels have not been updated for split
    // mask implimentation.");
    ...
    if (num_out_channels % 64 == 0 && num_in_channels % 32 == 0) {
      ...
      // threadIdx.y: i_factors[2] * j_factors[2]
      dim3 threads_per_block(32, 2);
      conv_backward_cuda_setting2_mode1_tf32tf32f32<<<num_blocks,
                                                      threads_per_block>>>(
          _kernel.size(0), num_in_channels, num_out_channels, kernel_volume,
          split_k_iters, split_mask_len, reduced_mask_len, reorder_loc_len,
          in_feats, kernel, reduced_mask, out_in_map, reorder_loc, out_feats);
      ...
      // threadIdx.y: i_factors[2] * j_factors[2]
      dim3 threads_per_block(32, 1);
      // conv_backward_cuda_setting1_mode1_tf32tf32f32<<<num_blocks,
      // threads_per_block>>>(
      //     _kernel.size(0), num_in_channels, num_out_channels, kernel_volume,
      //     split_k_iters, in_feats, kernel, reduced_mask, out_in_map,
      //     reorder_loc, out_feats);
  ...
  } else  // fp32fp32fp32
  {
    ...
    if (num_out_channels % 64 == 0 && num_in_channels % 32 == 0) {
      ...
      conv_backward_cuda_setting2_mode1_f32f32f32<<<num_blocks,
                                                    threads_per_block>>>(
          _kernel.size(0), num_in_channels, num_out_channels, kernel_volume,
          split_k_iters, split_mask_len, reduced_mask_len, reorder_loc_len,
          in_feats, kernel, reduced_mask, out_in_map, reorder_loc, out_feats);
      ...
      //     _kernel.size(0), num_in_channels, num_out_channels, kernel_volume,
      //     split_k_iters, split_mask_len, reduced_mask_len, reorder_loc_len,
      //     in_feats, kernel, reduced_mask, out_in_map, reorder_loc,
      //     out_feats);
      if (num_in_channels % 16 == 0) {
        if (num_out_channels % 16 == 0) {
