...
__global__ void __launch_bounds__(64)
    conv_forward_cuda_setting1_mode0_f16f16f32(int M, int K_original, int N,
                                               int kernel_volume,
                                               half *__restrict__ A,
                                               half *__restrict__ B,
  ...
  // int *out_in_map_ptr = out_in_map + (blockIdx.x / j_factors1 * 128 +
  // threadIdx.y * 16 + threadIdx.x / 2) * kernel_volume + ((threadIdx.y * 256)
  // % 16) / K_original + ((threadIdx.x * 8) % 16) / K_original;
  int *out_in_map_ptr =
      out_in_map +
      (blockIdx.x / j_factors1 * 128 + threadIdx.y * 16 + threadIdx.x / 2) *
          kernel_volume +
      ((threadIdx.y * 256) % 16) / K_tile_padded +
      ((threadIdx.x * 8) % 16) / K_tile_padded;
  // half *A_ptr = A + ((threadIdx.y * 256 % 16) % K_original) + ((threadIdx.x *
  // 8 % 16) % K_original);
  half *A_ptr = A + ((threadIdx.y * 256 % 16) % K_tile_padded) +
                ((threadIdx.x * 8 % 16) % K_tile_padded);
  half *B_ptr = B + (blockIdx.x % j_factors1) * 16 +
                threadIdx.y * 256 / 16 * N + threadIdx.x * 8 / 16 * N +
                (threadIdx.x * 8) % 16;
  int reorder_loc_offset = blockIdx.x / j_factors1 * 8 * 16 +
  ...
  int B_ld_start, B_ld_amount, B_ld_bound, B_pred_guard, B_ld_amount_N,
      B_ld_K_bound;
  bool B_ld_K;
...

  {
    if constexpr (K_ld_check) {
      ...
      A_pred_guard = 0;
      for (int i = 0; i < A_ld_bound; i++) A_pred_guard |= (1 << i);
    } else {
      A_pred_guard = 1;
    }
    ...
    // int *out_in_map_ptr_local = out_in_map_ptr + i2_0_0 * K_tile /
    // K_original; half *A_ptr_local = A_ptr + (i2_0_0 * K_tile % K_original);
    int *out_in_map_ptr_local =
        out_in_map_ptr + i2_0_0 * K_tile / K_tile_padded;
    ...
    for (int ax0_ax1_fused_0 = 0; ax0_ax1_fused_0 < 4; ++ax0_ax1_fused_0) {
      ...
      // int input_idx = out_in_map_ptr_local[ax0_ax1_fused_0 * 32 *
      // kernel_volume + (ax0_ax1_fused_0 * 512 % 16) / K_original];
      int input_idx =
          out_in_map_ptr_local[ax0_ax1_fused_0 * 32 * kernel_volume +
                               (ax0_ax1_fused_0 * 512 % 16) / K_tile_padded];

      if (input_idx != -1) {
        /*
         *(uint4 *)(A_shared + ((((ax0_ax1_fused_0 * 1280) + (((int)threadIdx.y)
         ** 640)) + ((((int)threadIdx.x) >> 1) * 40)) + ((((int)threadIdx.x) &
         *1) * 8))) = (uint4 *)(A_ptr_local + input_idx * K_original +
         *((ax0_ax1_fused_0 * 512 % 16) % K_original));
         */
        uint4 A_loaded = make_uint4(0, 0, 0, 0);
        // global_load<K_ld_factor>(A_loaded, A_ptr_local + input_idx *
        // K_original + ((ax0_ax1_fused_0 * 512 % 16) % K_original),
        // A_pred_guard);
    ...
    if (threadIdx.y == 0) {
      /*
       *(uint4 *)(B_shared + (((((int)threadIdx.y) * 640) + ((((int)threadIdx.x)
       *>> 1) * 40)) + ((((int)threadIdx.x) & 1) * 8))) = (uint4
       **)(B_ptr_local);
       */
...
#pragma message("FP16 kernels will not be compiled for SM75-.")
  ...
  for (int ax0_0_1 = 0; ax0_0_1 < 4; ++ax0_0_1) {
    ...
    for (int local_id = 0; local_id < 8; ++local_id) {
      // original:
      // (&(C[(((((((int)blockIdx.x) * 8192) + ((((int)threadIdx.y) & 1) *
      // 4096)) + (ax0_0_1 * 1024)) + ((((int)threadIdx.y) >> 1) * 32)) +
      // (ax1_0_1 * 16))]))[((((((local_id / 2) % 2) * 8) + (threadIdx.x / 4)) *
      // 64) + (((local_id % 2) + ((local_id / 4) * 8)) + ((threadIdx.x % 4) *
      // 2)))]
      int reorder_location_cur =
          reorder_loc_offset_local + (((local_id / 2) % 2) * 8);
      if constexpr (N_ld_check) {
        bool C_wb_enable =
            ((blockIdx.x % j_factors1) * 16 + threadIdx.y / 2 * 16 +
             (threadIdx.x % 4) * 2 + (local_id % 2) + (local_id / 4) * 8) < N;
...
__global__ void __launch_bounds__(64)
    conv_forward_cuda_setting2_mode0_f16f16f32(int M, int K_original, int N,
                                               int kernel_volume,
                                               half *__restrict__ A,
                                               half *__restrict__ B,
                                               int *__restrict__ out_in_map,
  ...
  int *out_in_map_ptr =
      out_in_map +
      (blockIdx.x / j_factors1 * 128 + threadIdx.y * 8 + threadIdx.x / 4) *
          kernel_volume +
...

  {
    ...
    for (int ax0_ax1_fused_0 = 0; ax0_ax1_fused_0 < 8; ++ax0_ax1_fused_0) {
      ...
      // Haotian: NOTE: what if j_factors[0] != 1?
      // original:
      // int input_idx = out_in_map[(((((((int)blockIdx.x) * 3456) +
      // (ax0_ax1_fused_0 * 864)) + (((int)threadIdx.y) * 216)) +
      // ((((int)threadIdx.x) >> 2) * 27)) + (i2_0_0 >> 1))];
      int input_idx =
          out_in_map_ptr_local[ax0_ax1_fused_0 * 16 * kernel_volume +
...
__global__ void __launch_bounds__(128)
    conv_forward_cuda_setting3_mode0_f16f16f32(int M, int K_original, int N,
                                               int kernel_volume,
                                               half *__restrict__ A,
                                               half *__restrict__ B,
                                               int *__restrict__ out_in_map,
  ...
  half *A_ptr = A + ((threadIdx.y * 256 % 32) % K_original) +
                ((threadIdx.x * 8 % 32) % K_original);
  half *B_ptr = B + (blockIdx.x % j_factors1) * 64 +
...
template <int K_ld_factor, int N_ld_factor, bool K_ld_check, bool N_ld_check>
__global__ void __launch_bounds__(64)
    conv_forward_cuda_setting1_mode0_tf32tf32f32(int M, int K_original, int N,
                                                 int kernel_volume,
  ...
  int *out_in_map_ptr =
      ...
      ((threadIdx.y * 256) % 16) / K_tile_padded +
      ((threadIdx.x * 8) % 16) / K_tile_padded;
  // float *A_ptr = A + ((threadIdx.y * 256 % 16) % K_original) + ((threadIdx.x
  // * 8 % 16) % K_original);
  float *A_ptr = A + ((threadIdx.y * 256 % 16) % K_tile_padded) +
                 ((threadIdx.x * 8 % 16) % K_tile_padded);
  float *B_ptr = B + (blockIdx.x % j_factors1) * 16 +
                 threadIdx.y * 256 / 16 * N + threadIdx.x * 8 / 16 * N +
                 (threadIdx.x * 8) % 16;
  int reorder_loc_offset = blockIdx.x / j_factors1 * 8 * 16 +
                           (threadIdx.y % 2) * 4 * 16 + (threadIdx.x / 4);
...

  {
    ...
    if (threadIdx.y == 0) {
      ...
      // ((((int)threadIdx.x) >> 1) * 40)) + ((((int)threadIdx.x) & 1) * 8))) =
    ...
    for (int local_size_1 = 0; local_size_1 < 8; ++local_size_1) {
      B_shared_warp[local_size_1] = B_shared[(
          ((((local_size_1 & 3) * 160) + ((((int)threadIdx.x) & 3) * 40)) +
           ((local_size_1 >> 2) * 8)) +
          (((int)threadIdx.x) >> 2))];
    }
...
__global__ void __launch_bounds__(64)
    conv_forward_cuda_setting2_mode0_tf32tf32f32(int M, int K_original, int N,
                                                 int kernel_volume,
                                                 float *__restrict__ A,
                                                 float *__restrict__ B,
                                                 int *__restrict__ out_in_map,
...
__global__ void __launch_bounds__(128)
    conv_forward_cuda_setting3_mode0_tf32tf32f32(int M, int K_original, int N,
                                                 int kernel_volume,
                                                 float *__restrict__ A,
                                                 float *__restrict__ B,
                                                 int *__restrict__ out_in_map,
  ...
  float *A_ptr = A + ((threadIdx.y * 256 % 32) % K_original) +
                 ((threadIdx.x * 8 % 32) % K_original);
  float *B_ptr = B + (blockIdx.x % j_factors1) * 64 +
                 threadIdx.y * 256 / 64 * N + threadIdx.x * 8 / 64 * N +
                 (threadIdx.x * 8) % 64;
  int reorder_loc_offset = blockIdx.x / j_factors1 * 8 * 16 +
                           (threadIdx.y % 2) * 4 * 16 + (threadIdx.x / 4);
...
template <int K_ld_factor, int N_ld_factor, bool K_ld_check, bool N_ld_check>
__global__ void __launch_bounds__(64)
    conv_forward_cuda_setting1_mode0_f32f32f32(int M, int K_original, int N,
                                               int kernel_volume,
                                               float *__restrict__ A,
                                               float *__restrict__ B,
                                               int *__restrict__ out_in_map,
  ...
  for (int k_0 = 0; k_0 < K_loops; ++k_0) {
    {
      if constexpr (K_ld_check) {
        A_ld_start = (k_0 * K_tile % K_tile_padded) +
                     ((threadIdx.x * 4) % 16);  // Channel_offset
...
#pragma unroll
      for (int ax0_ax1_fused_0 = 0; ax0_ax1_fused_0 < 8; ++ax0_ax1_fused_0) {
        ...
        if (input_idx != -1) {
          ...
          // ax0_ax1_fused_0 * elements loaded in each loop
...
#pragma unroll
      for (int ax0_ax1_fused_0_1 = 0; ax0_ax1_fused_0_1 < 1;
           ++ax0_ax1_fused_0_1) {
...
#pragma unroll
  for (int i = 0; i < 32; ++i) {
    ...
    if constexpr (N_ld_check) {
      if (vn < N && location_cur < M) C[location_cur * N + vn] = C_local[i];
    } else {
      if (location_cur < M) C[location_cur * N + vn] = C_local[i];
    }
  }
}
...
__global__ void __launch_bounds__(64)
    conv_forward_cuda_setting2_mode0_f32f32f32(int M, int K_original, int N,
                                               int kernel_volume,
                                               float *__restrict__ A,
                                               float *__restrict__ B,
                                               int *__restrict__ out_in_map,
...
__global__ void __launch_bounds__(128)
    conv_forward_cuda_setting3_mode0_f32f32f32(int M, int K_original, int N,
                                               int kernel_volume,
                                               float *__restrict__ A,
                                               float *__restrict__ B,
                                               int *__restrict__ out_in_map,
...
at::Tensor conv_forward_implicit_gemm_cuda(
    ...
    int num_out_feats, int num_out_channels, bool allow_tf32, bool allow_fp16) {
  ...
  at::Tensor _out_feats =
      torch::empty({num_out_feats, num_out_channels}, options);
  ...
  if (is_half) {
    ...
    if (num_out_channels % 64 == 0 && num_in_channels % 32 == 0) {
      ...
      conv_forward_cuda_setting3_mode0_f16f16f32<<<num_blocks,
                                                   threads_per_block>>>(
      ...
      conv_forward_cuda_setting2_mode0_f16f16f32<<<num_blocks,
                                                   threads_per_block>>>(
          _out_feats.size(0), num_in_channels, num_out_channels, kernel_volume,
          in_feats, kernel, out_in_map, out_feats);
    } else {
      ...
      // threadIdx.y: i_factors[2] * j_factors[2]
      dim3 threads_per_block(32, 2);
      // conv_forward_cuda_setting1_mode0_f16f16f32<<<num_blocks,
      // threads_per_block>>>(
      //     _out_feats.size(0), num_in_channels, num_out_channels,
    ...
    if (num_out_channels % 64 == 0 && num_in_channels % 32 == 0) {
      ...
      // threadIdx.y: i_factors[2] * j_factors[2]
      dim3 threads_per_block(32, 4);
      conv_forward_cuda_setting3_mode0_tf32tf32f32<<<num_blocks,
                                                     threads_per_block>>>(
          _out_feats.size(0), num_in_channels, num_out_channels, kernel_volume,
          in_feats, kernel, out_in_map, out_feats);
    } else if (num_in_channels % 32 == 0 && num_out_channels % 16 == 0) {
      ...
      // threadIdx.y: i_factors[2] * j_factors[2]
      dim3 threads_per_block(32, 2);
      conv_forward_cuda_setting2_mode0_tf32tf32f32<<<num_blocks,
                                                     threads_per_block>>>(
          _out_feats.size(0), num_in_channels, num_out_channels, kernel_volume,
          in_feats, kernel, out_in_map, out_feats);
    } else {
      ...
      // threadIdx.y: i_factors[2] * j_factors[2]
      dim3 threads_per_block(32, 2);
      // conv_forward_cuda_setting1_mode0_tf32tf32f32<<<num_blocks,
      // threads_per_block>>>(
      //     _out_feats.size(0), num_in_channels, num_out_channels,
      //     kernel_volume, in_feats, kernel, out_in_map, out_feats);
      if (num_in_channels % 16 == 0) {
  ...
  } else  // fp32fp32fp32
  {
    ...
    if (num_out_channels % 64 == 0 && num_in_channels % 32 == 0) {
      ...
      conv_forward_cuda_setting3_mode0_f32f32f32<<<num_blocks,
                                                   threads_per_block>>>(
          _out_feats.size(0), num_in_channels, num_out_channels, kernel_volume,
          in_feats, kernel, out_in_map, out_feats);
    } else if (num_in_channels % 32 == 0 && num_out_channels % 16 == 0) {
      ...
      conv_forward_cuda_setting2_mode0_f32f32f32<<<num_blocks,
                                                   threads_per_block>>>(
          _out_feats.size(0), num_in_channels, num_out_channels, kernel_volume,
          in_feats, kernel, out_in_map, out_feats);
    } else {
