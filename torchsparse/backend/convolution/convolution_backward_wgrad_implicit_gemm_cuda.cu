...
__global__ void __launch_bounds__(32)
    conv_backward_cuda_setting1_mode0_f16f16f32(
  ...
  int blockIdx_x = 0;
  int blockIdx_y =
      blockIdx.x % ((K_original + 15) / 16 * kernel_volume * j_factors1);
  int blockIdx_z =
      blockIdx.x / ((K_original + 15) / 16 * kernel_volume * j_factors1);
  ...
  // int *out_in_map_ptr = out_in_map + (threadIdx.y * 16 + threadIdx.x / 2) *
  // kernel_volume + ((threadIdx.y * 256) % 16) / K_original + ((threadIdx.x *
  // 8) % 16) / K_original + (blockIdx_y / j_factors1 * 16) / K_original;
  int *out_in_map_ptr = out_in_map +
                        (threadIdx.y * 16 + threadIdx.x / 2) * kernel_volume +
  ...
  // half *C_ptr = cur_C + blockIdx_x / 1 * 108 * N / 16 * 256 + blockIdx_y /
  // j_factors1 * 1 * N / 16 * 256 + (threadIdx.y % 1) * 1 * N / 16 * 256 +
  // (blockIdx_x % 1) * j_factors1 * 16 + (blockIdx_y % j_factors1) * 16 +
  // threadIdx.y / 1 * 16 + (threadIdx.x % 4) * 2 + (threadIdx.x / 4) * N;
  int K_iters = ((M_fwd + 63) / 64 + split_k_iters - 1) / split_k_iters;
  // int kernel_offset = (blockIdx_y / j_factors1) / (K_original / 16);
  int kernel_offset =
      (blockIdx_y / j_factors1) / ((K_original + K_tile - 1) / K_tile);
  int cur_C_ic_start =
      (blockIdx_y / j_factors1 * 16) % K_tile_padded + (threadIdx.x / 4);
  int cur_C_oc_start = (blockIdx_y % j_factors1) * 16 + threadIdx.y / 1 * 16 +
                       (threadIdx.x % 4) * 2;
  half *C_ptr = cur_C + (kernel_offset * K_original + cur_C_ic_start) * N +
                cur_C_oc_start;
  ...
  for (int _i2_0_0 = 0; _i2_0_0 < K_iters - 1; ++_i2_0_0) {
    ...
    for (int ax0_ax1_fused_0 = 0; ax0_ax1_fused_0 < 4; ++ax0_ax1_fused_0) {
      ...
      // int input_idx = out_in_map_ptr_local[ax0_ax1_fused_0 * 16 *
      // kernel_volume + (ax0_ax1_fused_0 * 256 % 16) / K_original];
      int input_idx =
          out_in_map_ptr_local[ax0_ax1_fused_0 * 16 * kernel_volume +
                               (ax0_ax1_fused_0 * 256 % 16) / K_tile_padded];

      if (input_idx != -1) {
        //*(uint4 *)(A_shared + (((ax0_ax1_fused_0 * 640) + ((((int)threadIdx.x)
        ...
        global_load<K_ld_factor>(
            ...
            A_ptr_local + input_idx * K_original +
                ((ax0_ax1_fused_0 * 256 % 16) % K_tile_padded),
            A_pred_guard);
        *(uint4 *)(A_shared + (((ax0_ax1_fused_0 * 640) +
    ...
    if constexpr (K_ld_check || N_ld_check) {
      if (cur_C_ic_start + ((local_id / 2) % 2) * 8 < K_original &&
        ...
        C_ptr[+(((local_id / 2) % 2) * 8) * N + (local_id % 2) +
              (local_id / 4) * 8] = __float2half(C_warp[0 + local_id]);
    } else {
      C_ptr[+(((local_id / 2) % 2) * 8) * N + (local_id % 2) +
            (local_id / 4) * 8] = __float2half(C_warp[0 + local_id]);
    }
...
__global__ void __launch_bounds__(64)
  ...
  int blockIdx_x = 0;
  int blockIdx_y =
      blockIdx.x % ((K_original * kernel_volume + 31) / 32 * j_factors1);
  int blockIdx_z =
      blockIdx.x / ((K_original * kernel_volume + 31) / 32 * j_factors1);
  ...
  // hoisting shared pointer offsets
  int *out_in_map_ptr = out_in_map +
                        (threadIdx.y * 8 + threadIdx.x / 4) * kernel_volume +
  ...
  half *C_ptr = cur_C + blockIdx_x / 1 * 108 * N / 16 * 256 +
                ...
                (blockIdx_y % j_factors1) * 64 + threadIdx.y / 1 * 32 +
                (threadIdx.x % 4) * 2 + (threadIdx.x / 4) * N;
  int K_iters = ((M_fwd + 63) / 64 + split_k_iters - 1) / split_k_iters;
  ...
  for (int _i2_0_0 = 0; _i2_0_0 < K_iters - 1; ++_i2_0_0) {
    ...
    for (int ax0_ax1_fused_0_1 = 0; ax0_ax1_fused_0_1 < 8;
         ++ax0_ax1_fused_0_1) {
      ...
      for (int ax1_0_1 = 0; ax1_0_1 < 2; ++ax1_0_1) {
        {
          ...
          __asm__ __volatile__(
              ...
              : "=r"(addr)
              : "l"((void *)((&(B_shared[(
                                 ((i2_0_1 * 1152) + (((int)threadIdx.y) * 32)) +
                                 (ax1_0_1 * 16))])) +
...
template <int K_ld_factor, int N_ld_factor, bool K_ld_check, bool N_ld_check>
__global__ void __launch_bounds__(32)
    conv_backward_cuda_setting1_mode0_tf32tf32f32(
        int M_fwd, int K_original, int N, int kernel_volume, int split_k_iters,
        float *__restrict__ A, float *__restrict__ B,
        int *__restrict__ out_in_map, float *__restrict__ C) {
  int j_factors1 = (N + 15) / 16 / 1;
  int blockIdx_x = 0;
  int blockIdx_y =
      blockIdx.x % ((K_original + 15) / 16 * kernel_volume * j_factors1);
  ...
  int reorder_offset = threadIdx.y * 256 / 16 + threadIdx.x * 8 / 16;
  // float *C_ptr = cur_C + blockIdx_x / 1 * 108 * N / 16 * 256 + blockIdx_y /
  // j_factors1 * 1 * N / 16 * 256 + (threadIdx.y % 1) * 1 * N / 16 * 256 +
  // (blockIdx_x % 1) * j_factors1 * 16 + (blockIdx_y % j_factors1) * 16 +
  // threadIdx.y / 1 * 16 + (threadIdx.x % 4) * 2 + (threadIdx.x / 4) * N;
  int K_iters = ((M_fwd + 63) / 64 + split_k_iters - 1) / split_k_iters;
  ...
  int cur_C_oc_start = (blockIdx_y % j_factors1) * 16 + threadIdx.y / 1 * 16 +
                       (threadIdx.x % 4) * 2;
  float *C_ptr = cur_C + (kernel_offset * K_original + cur_C_ic_start) * N +
                 cur_C_oc_start;
  ...
  for (int _i2_0_0 = 0; _i2_0_0 < K_iters - 1; ++_i2_0_0) {
    ...
    for (int ax0_ax1_fused_0_1 = 0; ax0_ax1_fused_0_1 < 4;
      ...
      int reorder_offset_inner = reorder_offset_local + ax0_ax1_fused_0_1 * 16;
      int v0 = reorder_offset_inner;
      //*(ulonglong4 *)(B_shared + (((ax0_ax1_fused_0_1 * 640) +
      //((((int)threadIdx.x) >> 1) * 40)) + ((((int)threadIdx.x) & 1) * 8))) =
  ...
  for (int local_id = 0; local_id < 8; ++local_id) {
    if constexpr (K_ld_check || N_ld_check) {
      if (cur_C_ic_start + ((local_id / 2) % 2) * 8 < K_original &&
          cur_C_oc_start + (local_id % 2) + (local_id / 4) * 8 < N)
        C_ptr[+(((local_id / 2) % 2) * 8) * N + (local_id % 2) +
              (local_id / 4) * 8] = C_warp[0 + local_id];
    } else {
      C_ptr[+(((local_id / 2) % 2) * 8) * N + (local_id % 2) +
            (local_id / 4) * 8] = C_warp[0 + local_id];
    }
...
__global__ void __launch_bounds__(64)
  ...
  for (int _i2_0_0 = 0; _i2_0_0 < K_iters - 1; ++_i2_0_0) {
    ...
    for (int ax0_ax1_fused_0_1 = 0; ax0_ax1_fused_0_1 < 8;
      ...
      int reorder_offset_inner = reorder_offset_local + ax0_ax1_fused_0_1 * 8;
      int v0 = reorder_offset_inner;
      *(ulonglong4 *)(B_shared + ((((ax0_ax1_fused_0_1 * 576) +
                                    (((int)threadIdx.y) * 288)) +
                                   ((((int)threadIdx.x) >> 3) * 72)) +
                                  ((((int)threadIdx.x) & 7) * 8))) =
          *(ulonglong4 *)(B_ptr + v0 * N);
...
template <int K_ld_factor, int N_ld_factor, bool K_ld_check, bool N_ld_check>
__global__ void __launch_bounds__(32)
    conv_backward_cuda_setting1_mode0_f32f32f32(
        int M_fwd, int K_original, int N, int kernel_volume, int split_k_iters,
        float *__restrict__ A, float *__restrict__ B,
        int *__restrict__ out_in_map, float *__restrict__ C) {
...
#pragma unroll
  ...
  int channel_offset =
      (blockIdx_m * 16 + ((threadIdx_x * 4) % 16)) % K_tile_padded;
  int K_loops = ((M_fwd + 63) / 64 + split_k_iters - 1) / split_k_iters;

  // hoisting shared pointer offsets
  int *out_in_map_ptr =
      out_in_map + (threadIdx_x / (16 / 4)) * kernel_volume + kernel_offset;
...
#pragma unroll
    for (int ax0_ax1_fused_0 = 0; ax0_ax1_fused_0 < 8; ++ax0_ax1_fused_0) {
      int input_idx =
          out_in_map_ptr_local[(ax0_ax1_fused_0 * 8) * kernel_volume];
      if (input_idx != -1) {
        // *(float4*)(A_shared_ptr + (ax0_ax1_fused_0 * 128)) =  //
        // ax0_ax1_fused_0 * elements loaded in each loop
        //     *(float4*)(A_ptr + (input_idx * K_original));
        uint4 A_loaded = make_uint4(0, 0, 0, 0);
        global_load<K_ld_factor>(A_loaded, A_ptr + (input_idx * K_original),
                                 A_pred_guard);
        *(uint4 *)(A_shared_ptr + (ax0_ax1_fused_0 * 128)) = A_loaded;
      } else {
        *(uint4 *)(A_shared_ptr + (ax0_ax1_fused_0 * 128)) =
            make_uint4(0, 0, 0, 0);
      }
    }
...
#pragma unroll
    for (int ax0_ax1_fused_0_1 = 0; ax0_ax1_fused_0_1 < 8;
      ...
      if (reorder_offset_inner < M_fwd) {
        ...
        *(uint4 *)(B_shared_ptr + (ax0_ax1_fused_0_1 * 128)) =
            make_uint4(0, 0, 0, 0);
      }
    }
...
#pragma unroll
  for (int i = 0; i < 8; ++i) {
    ...
    int local_col = ((i % 4) * 4);
    if constexpr (K_ld_check || N_ld_check) {
      if (((cur_C_ic_start + local_row) < K_original) &&
          ((cur_C_oc_start + local_col) < N))
...
#pragma unroll
  ...
  int channel_offset =
      (blockIdx_m * 32 + ((threadIdx_x * 4) % 32)) % K_original;
  int K_loops = ((M_fwd + 63) / 64 + split_k_iters - 1) / split_k_iters;

  // hoisting shared pointer offsets
  int *out_in_map_ptr =
      out_in_map + (threadIdx_x / (32 / 4)) * kernel_volume + kernel_offset;
  float *A_ptr = A + channel_offset;
...
#pragma unroll
    for (int ax0_ax1_fused_0_1 = 0; ax0_ax1_fused_0_1 < 16;
         ++ax0_ax1_fused_0_1) {
...
at::Tensor conv_backward_wgrad_implicit_gemm_cuda(
    ...
    const int split_k_iters, bool allow_tf32, bool allow_fp16) {
  ...
  if (is_half) {
    ...
    if (num_out_channels % 64 == 0 && num_in_channels % 32 == 0) {
      ...
      dim3 num_blocks(num_in_channels * kernel_volume / 32 * j_factors1 *
                      split_k_iters);
      // threadIdx.x: 32
      // threadIdx.y: i_factors[2] * j_factors[2]
      dim3 threads_per_block(32, 2);
      conv_backward_cuda_setting2_mode0_f16f16f32<<<num_blocks,
                                                    threads_per_block>>>(
      ...
      // conv_backward_cuda_setting1_mode0_f16f16f32<<<num_blocks,
      // threads_per_block>>>(
      //     _kernel.size(0), num_in_channels, num_out_channels, kernel_volume,
      //     split_k_iters, in_feats, kernel, out_in_map, out_feats);
        ...
        if (num_out_channels % 16 == 0) {
          conv_backward_cuda_setting1_mode0_f16f16f32<16, 16, false, false>
              <<<num_blocks, threads_per_block>>>(
                  _kernel.size(0), num_in_channels, num_out_channels,
                  kernel_volume, split_k_iters, in_feats, kernel, out_in_map,
                  out_feats);
    ...
    if (num_out_channels % 64 == 0 && num_in_channels % 32 == 0) {
      ...
      // threadIdx.y: i_factors[2] * j_factors[2]
      dim3 threads_per_block(32, 2);
      conv_backward_cuda_setting2_mode0_tf32tf32f32<<<num_blocks,
                                                      threads_per_block>>>(
          _kernel.size(0), num_in_channels, num_out_channels, kernel_volume,
          split_k_iters, in_feats, kernel, out_in_map, out_feats);
    } else {
      ...
      // threadIdx.y: i_factors[2] * j_factors[2]
      dim3 threads_per_block(32, 1);
      // conv_backward_cuda_setting1_mode0_tf32tf32f32<<<num_blocks,
      // threads_per_block>>>(
      //     _kernel.size(0), num_in_channels, num_out_channels, kernel_volume,
      //     split_k_iters, in_feats, kernel, out_in_map, out_feats);
      if (num_in_channels % 16 == 0) {
  ...
  } else  // fp32fp32fp32
  {
    ...
    if (num_out_channels % 64 == 0 && num_in_channels % 32 == 0) {
      ...
      conv_backward_cuda_setting2_mode0_f32f32f32<<<num_blocks,
                                                    threads_per_block>>>(
          _kernel.size(0), num_in_channels, num_out_channels, kernel_volume,
          split_k_iters, in_feats, kernel, out_in_map, out_feats);
    } else {
