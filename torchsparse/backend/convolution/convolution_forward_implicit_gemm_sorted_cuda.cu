...
__global__ void __launch_bounds__(64)
  ...
  for (int i2_0_0 = K_st * K_tile_padded / K_tile;
       i2_0_0 < K_ed * K_tile_padded / K_tile; ++i2_0_0)
...
__global__ void __launch_bounds__(64)
  ...
  for (int i2_0_0 = K_st * K_original / 32; i2_0_0 < K_ed * K_original / 32;
       ++i2_0_0)

  {
    ...
    if (bit_flag) {
      ...
      for (int ax0_ax1_fused_0 = 0; ax0_ax1_fused_0 < 8; ++ax0_ax1_fused_0) {
        ...
        // original:
        // int input_idx = out_in_map[(((((((int)blockIdx_y) * 3456) +
...
__global__ void __launch_bounds__(128)
  ...
  half *C_ptr = C +
                ...
                (blockIdx_y % j_factors1) * 64 + threadIdx.y / 2 * 32 +
                (threadIdx.x % 4) * 2;
  //+ (threadIdx.x / 4) * N;
...
#pragma message("FP16 kernels will not be compiled for SM75-.")
  ...
  for (int ax0_0_1 = 0; ax0_0_1 < 4; ++ax0_0_1) {
    ...
    for (int ax1_0_1 = 0; ax1_0_1 < 2; ++ax1_0_1) {
      for (int local_id = 0; local_id < 8; ++local_id) {
        // original:
        // (&(C[(((((((int)blockIdx_y) * 8192) + ((((int)threadIdx.y) & 1) *
...
at::Tensor conv_forward_implicit_gemm_sorted_cuda(
  ...
  if (split_mask_num != 1)
    _out_feats = torch::empty({split_mask_num, num_out_feats, num_out_channels},
                              options);
  else
  ...
  if (is_half) {
    ...
    if (num_out_channels % 64 == 0 && num_in_channels % 32 == 0) {
      ...
      dim3 num_blocks(1 * (num_out_feats + 127) / 128 * j_factors1 *
                      split_mask_num);
      // threadIdx.x: 32
      // threadIdx.y: i_factors[2] * j_factors[2]
      dim3 threads_per_block(32, 4);
      conv_forward_cuda_setting3_mode1_f16f16f32<<<num_blocks,
                                                   threads_per_block>>>(
      ...
      conv_forward_cuda_setting2_mode1_f16f16f32<<<num_blocks,
                                                   threads_per_block>>>(
          num_out_feats, num_in_channels, num_out_channels, kernel_volume,
          split_mask_len, _reduced_mask.size(1), _reorder_loc.size(1), in_feats,
          kernel, reduced_mask, out_in_map, reorder_loc, out_feats);
      ...
      // threadIdx.y: i_factors[2] * j_factors[2]
      dim3 threads_per_block(32, 2);
      // conv_forward_cuda_setting1_mode1_f16f16f32<<<num_blocks,
      // threads_per_block>>>(
      //     _out_feats.size(0), num_in_channels, num_out_channels,
      //     kernel_volume, in_feats, kernel, reduced_mask, out_in_map,
        ...
        if (num_out_channels % 16 == 0) {
          conv_forward_cuda_setting1_mode1_f16f16f32<16, 16, false, false>
              <<<num_blocks, threads_per_block>>>(
                  ...
                  kernel_volume, split_mask_len, _reduced_mask.size(1),
                  _reorder_loc.size(1), in_feats, kernel, reduced_mask,
    ...
    if (num_out_channels % 64 == 0 && num_in_channels % 32 == 0) {
      ...
      // threadIdx.y: i_factors[2] * j_factors[2]
      dim3 threads_per_block(32, 4);
      conv_forward_cuda_setting3_mode1_tf32tf32f32<<<num_blocks,
                                                     threads_per_block>>>(
          num_out_feats, num_in_channels, num_out_channels, kernel_volume,
          split_mask_len, _reduced_mask.size(1), _reorder_loc.size(1), in_feats,
          kernel, reduced_mask, out_in_map, reorder_loc, out_feats);
      ...
      // threadIdx.y: i_factors[2] * j_factors[2]
      dim3 threads_per_block(32, 2);
      conv_forward_cuda_setting2_mode1_tf32tf32f32<<<num_blocks,
                                                     threads_per_block>>>(
          num_out_feats, num_in_channels, num_out_channels, kernel_volume,
          split_mask_len, _reduced_mask.size(1), _reorder_loc.size(1), in_feats,
          kernel, reduced_mask, out_in_map, reorder_loc, out_feats);
      ...
      // threadIdx.y: i_factors[2] * j_factors[2]
      dim3 threads_per_block(32, 2);
      // conv_forward_cuda_setting1_mode1_tf32tf32f32<<<num_blocks,
      // threads_per_block>>>(
      //     _out_feats.size(0), num_in_channels, num_out_channels,
      //     kernel_volume, in_feats, kernel, reduced_mask, out_in_map,
      //     reorder_loc, out_feats);
  ...
  } else  // fp32fp32fp32
  {
    ...
    if (num_out_channels % 64 == 0 && num_in_channels % 32 == 0) {
      ...
      conv_forward_cuda_setting3_mode1_f32f32f32<<<num_blocks,
                                                   threads_per_block>>>(
          num_out_feats, num_in_channels, num_out_channels, kernel_volume,
      ...
      conv_forward_cuda_setting2_mode1_f32f32f32<<<num_blocks,
                                                   threads_per_block>>>(
          num_out_feats, num_in_channels, num_out_channels, kernel_volume,
          split_mask_len, reduced_mask_len, reorder_loc_len, in_feats, kernel,
          reduced_mask, out_in_map, reorder_loc, out_feats);
