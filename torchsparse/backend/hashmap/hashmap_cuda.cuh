...
class GPUHashTable {
 ...
 public:
  ...
  torch::Tensor lookup_coords(at::Tensor coords, at::Tensor kernel_sizes,
                              at::Tensor tensor_strides, int kernel_volume);
  ...
  class device_view {
   ...
   public:
    __host__ __device__ device_view(int capacity, key_type* table_keys,
                                    val_type* table_vals)
        : _capacity(capacity),
          _table_keys(table_keys),
          _table_vals(table_vals) {}
...
__global__ void insert_kernel(key_type* table_keys, val_type* table_vals,
                              const key_type* keys, int n, int _capacity) {
...
template <typename key_type = int64_t, typename val_type = int>
__global__ void insert_coords_kernel(key_type* table_keys, val_type* table_vals,
                                     int* coords, int n, int _capacity) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx < n) {
...
// lookup from hashmap
template <typename key_type = int64_t, typename val_type = int>
__global__ void lookup_kernel(key_type* table_keys, val_type* table_vals,
                              const key_type* keys, val_type* vals, int n,
                              int _capacity) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
  if (idx < n) {
    key_type key = keys[idx];
...
__global__ void lookup_coords_kernel(key_type* table_keys, val_type* table_vals,
                                     int* coords, val_type* vals,
                                     const int* kernel_sizes,
                                     const int* strides, int n, int _capacity,
                                     int kernel_volume) {
...
template <typename key_type, typename val_type>
void GPUHashTable<key_type, val_type>::insert_many(const key_type* keys,
                                                   const int n) {
  insert_kernel<key_type, val_type>
      <<<(n + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE>>>(
          table_keys, table_vals, keys, n, _capacity);
}

template <typename key_type, typename val_type>
void GPUHashTable<key_type, val_type>::insert_many_coords(int* coords,
                                                          const int n) {
...
template <typename key_type, typename val_type>
void GPUHashTable<key_type, val_type>::lookup_many(const key_type* keys,
                                                   val_type* results,
                                                   const int n) {
  lookup_kernel<key_type, val_type>
      <<<(n + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE>>>(
...
void GPUHashTable<key_type, val_type>::lookup_many_coords(
  ...
  if (kernel_volume % 2)
    lookup_coords_kernel<key_type, val_type, true>
        <<<(n * kernel_volume + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE>>>(
            table_keys, table_vals, coords, results, kernel_sizes, strides, n,
            _capacity, kernel_volume);
  else
...
at::Tensor GPUHashTable<key_type, val_type>::lookup_vals(at::Tensor keys) {
  ...
  at::Tensor results = torch::zeros(
      {(keys.size(0) + _divisor - 1) / _divisor * _divisor}, options);
  lookup_many(keys.data_ptr<key_type>(), results.data_ptr<val_type>(),
              keys.size(0));
  return results;
}

template <typename key_type, typename val_type>
at::Tensor GPUHashTable<key_type, val_type>::lookup_coords(
    at::Tensor coords, at::Tensor kernel_sizes, at::Tensor strides,
    int kernel_volume) {
  auto options =
      torch::TensorOptions().dtype(at::ScalarType::Int).device(coords.device());
  at::Tensor results = torch::zeros(
      {(coords.size(0) + _divisor - 1) / _divisor * _divisor, kernel_volume},
      options);
  lookup_many_coords(coords.data_ptr<int>(), results.data_ptr<val_type>(),
                     kernel_sizes.data_ptr<int>(), strides.data_ptr<int>(),
                     coords.size(0), kernel_volume);
  return results;
}

template <typename key_type, typename val_type>
__device__ void GPUHashTable<key_type, val_type>::device_view::insert(
    const key_type key, const val_type val) {
...
template <typename key_type, typename val_type>
__device__ val_type
GPUHashTable<key_type, val_type>::device_view::lookup(const key_type key) {
  int slot = hash_murmur3(key, _capacity);
