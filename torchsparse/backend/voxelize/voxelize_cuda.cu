...
__global__ void to_dense_forward_kernel(int N, int c,
  ...
  if (i < N) {
    ...
    int pos = cur_coords[0] * range[1] * range[2] * range[3] +
              cur_coords[1] * range[2] * range[3] + cur_coords[2] * range[3] +
              cur_coords[3];
...
template <typename scalar_t>
__global__ void to_dense_backward_kernel(int N, int c,
