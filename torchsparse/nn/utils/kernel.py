...
def get_kernel_offsets(
    ...
    if np.prod(size) % 2 == 1:
        offsets = tuple([(x, y, z) for z in offsets[2] for y in offsets[1]
                         for x in offsets[0]])
    else:
        offsets = tuple([(x, y, z) for x in offsets[0] for y in offsets[1]
                         for z in offsets[2]])

    offsets = make_tensor(offsets, dtype=torch.int, device=device)
    return offsets
