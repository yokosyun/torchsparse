...
def spupsample_generative(
    ...
    sample_stride = make_tensor(stride, dtype=torch.int,
                                device=_coords.device).unsqueeze(0)
    # stride and dilation are both 1
    kernel_offsets = get_kernel_offsets(kernel_size,
                                        1,
                                        1,
    ...
    assert (spatial_range
            is not None), "spatial range must be specified in generative mode"
