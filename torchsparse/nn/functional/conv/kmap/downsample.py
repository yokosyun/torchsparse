...
def spdownsample(
    ...
    sample_stride = make_tensor(sample_stride,
                                dtype=torch.int,
                                device=_coords.device).unsqueeze(dim=0)
    ...
    if (all(stride[k] in [1, kernel_size[k]] for k in range(3))
        ...
        if _coords.device.type == "cuda":
            ...
            padding_t = make_tensor(padding,
                                    dtype=torch.int,
                                    device=_coords.device)
            kernel_size_t = make_tensor(kernel_size,
                                        dtype=torch.int,
                                        device=_coords.device)
            stride_t = make_tensor(stride,
                                   dtype=torch.int,
                                   device=_coords.device)
